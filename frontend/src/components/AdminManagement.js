...
const AdminManagement = () => {
  ...
  const handleDownloadPDF = () => {
    ...
    // Add logo (top left) and college name (centered)
    const logoUrl = `${window.location.origin}/logo.png`;
    const collegeName = "College Of Engineering Guindy";
    ...
    img.onload = function () {
      ...
      doc.setFontSize(18);
      doc.text(collegeName, doc.internal.pageSize.getWidth() / 2, 18, {
        align: "center",
      });
      // Title below college name
      doc.setFontSize(16);
      doc.text("OD Requests", 14, 36);
      const tableColumn = [
        "Student Name",
        "Roll Number",
        "Department",
      ...
      autoTable(doc, { head: [tableColumn], body: tableRows, startY: 42 });
